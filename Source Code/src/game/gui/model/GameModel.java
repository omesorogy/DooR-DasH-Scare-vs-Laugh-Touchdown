...
public class GameModel {
    ...
    private int    lastCardsLeft    = -1;   // cards remaining after last draw
    private boolean lastWasReshuffle = false; // true if deck was reshuffled before last draw
    ...
    public int  getLastCardsLeft()       { return lastCardsLeft; }
    public boolean wasLastReshuffle()    { return lastWasReshuffle; }
    ...
    public String playTurn() {
        ...
        // If deck was reshuffled it resets to ~25; detect by size going UP or card drawn from empty deck
        if (cardsBefore == 0 || (cardsBefore > 0 && cardsAfter > cardsBefore)) {
            lastWasReshuffle = true;
        }
        lastCardsLeft = cardsAfter;
        ...
        // --- Card detection: a card was drawn if deck shrank this turn ---
        game.engine.cards.Card drawnCard = game.getLastDrawnCard();
        // cardsAfter < cardsBefore means exactly one card was consumed this turn.
        // Using size comparison is reliable; the loop-based cell scan was error-prone.
        boolean cardDrawnThisTurn = (drawnCard != null);
        ...
        if (cardDrawnThisTurn) {
            ...
            // Display the deck message after the drawn card has already been removed.
            // Example on first draw from a 25-card deck: 24 cards left.
            String deckInfo = "\nCard discarded, " + cardsAfter + " cards left in the deck.";
            if (lastWasReshuffle) {
                deckInfo = "  Deck reshuffled! " + deckInfo.trim();
            }
            lastCardEffect += deckInfo;
        }

        // Actual steps moved:
        // - Forward normal:    posAfter > posBefore  -> posAfter - posBefore
        // - Forward wrap:      dice roll pushes past 99 -> posAfter < posBefore but small posAfter
        //                      detected by: posBefore + lastDiceRoll*3 >= 100
        // - Backward move:     contamination sock / start-over / swap going back
        //                      detected by: posAfter < posBefore AND not a wrap
        int actualSteps;
        if (posAfter >= posBefore) {
            // Simple forward move
            actualSteps = posAfter - posBefore;
        } else {
            // posAfter < posBefore - could be wrap-around OR genuine backward
            boolean isWrap = (posBefore + lastDiceRoll * 3) >= 100;
            if (isWrap) {
                actualSteps = (posAfter - posBefore + 100) % 100;
            } else {
                // Genuine backward movement (negative value)
                actualSteps = posAfter - posBefore;
            }
        }
